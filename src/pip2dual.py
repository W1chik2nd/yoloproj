...
# 统一锁管理 - 使用单一全局锁避免死锁
global_lock = threading.RLock()  # 统一的全局锁，保护所有共享状态
gpu_lock = threading.RLock()     # 仅用于GPU推理操作
...
def get_next_cam2_assignment():
    """获取cam2新ID的分配 - 必须在global_lock内调用"""
    global cam0_right_exit_queue
...
def get_next_cam0_assignment():
    """获取cam0新ID的分配 - 必须在global_lock内调用"""
    global cam2_left_exit_queue
...
def cleanup_features(current_time):
    ...
    if len(cross_cam_features) > 50:
        # Sort by last seen time and remove the oldest ones
        sorted_sids = sorted(cross_cam_features, key=lambda s: cross_cam_features[s]['last_time'])
        sids_to_remove = sorted_sids[:len(cross_cam_features) - 50]
        for sid in sids_to_remove:
            cross_cam_features.pop(sid, None)

def try_acquire_control(cam_id, now):
    """尝试获取控制权 - 必须在global_lock内调用"""
...
def select_target_id(event, x, y, flags, param):
    ...
    if event == cv2.EVENT_LBUTTONDOWN:
        # 使用超时锁避免永久等待
        if global_lock.acquire(timeout=0.1):
            try:
...
def process_camera(cam_id):
    ...
    try:
        while True:
            ...
            # 使用超时锁处理主要逻辑
            if global_lock.acquire(timeout=0.05):  # 50ms超时
                try:
                    for track in tracks:
                                    ...
                                    if lx2 > lx1 and ly2 > ly1:
                                        ...
                                        if live_crop.size > 0:
                                            ...
                                            if time_since_lost > 0.5:
                                                cv2.putText(lost_view, f"Re-acquiring... ({remaining_time:.1f}s)", (20, 40), 
                ...
                print(f"[WARN] CAM{cam_id} 无法获取锁，跳过此帧")
                continue
